--
-- Impact Scenario 1 May 2018 View
--

--
-- View With Effective Duration (Rather Then Actuals)
--
create view impact_activity_may18 as 
SELECT 
concat('ImpM_', ICD.ACTUAL_CALLID) as ActivityKey,
DATE(ICD.CALL_STARTED_LOCAL) as OriginalStartDate, 
DATE(date_add(ICD.CALL_STARTED_LOCAL, INTERVAL + 483 DAY)) as StartDate,
DATE(DATE_ADD(date_add(ICD.CALL_STARTED_LOCAL, INTERVAL + 483 DAY), INTERVAL ICD.EFFECTIVE_DURATION_MINUTES MINUTE)) as EndDate,
...
select * from impact_activity_may18 limit 100;
...
select ActivityKey, ReqResource as ResourceId, activitytype, startdate, enddate, Latitude, Longitude, PlannedDuration as Duration, StartTime, EndTime, Store, city as City, state as State, zip as Zip, 'Eastern' as Timezone, TimeSlot as TimeSlot, Resource_No , DOW from impact_activity_may18 where startdate >= '2018-04-29' and startdate <= '2018-05-29';
